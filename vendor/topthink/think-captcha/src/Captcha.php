...
class Captcha
{
     ...
     */
    public function check($code, $id = '', $session_id = null)
    {
        $key = $this->authcode($this->seKey) . $id;

        if ($session_id) {
            session_id($session_id);// 仅设置 ID 不重新初始化
            session_start();
        }
     ...
     */
    public function entry($id = '', $type = 'image')
    {
        // 图片宽(px)
        ...
        if ($type == 'image') {
            ...
            return response([
                ...
                'msg'  => 'success',
                'data' => [
                    'captcha_image' => 'data:image/png;base64,' . $base64Content,
                    'captcha_key'    => $sessionId,
